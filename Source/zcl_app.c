...
#endif
#ifdef BIND_REQ
uint8 temp_bindingCount = 0;
uint8 temp_bindingStartIndex = 0;
#endif
...
static void EpdStatusLqiBatteryNwk(uint8 temp_s);
...
#endif
#ifdef LQI_REQ
void zclApp_RequestLqi(uint8 count);
#endif
#ifdef BIND_REQ
void zclApp_RequestBind(uint8 count);
#endif
...
//        uint16 eAd = BUILD_UINT16( eAd8[0],eAd8[1] );
        LREP("bc=%d bs=%d bl=%d\r\n", BindRsp->bindingCount, BindRsp->startIndex, BindRsp->bindingListCount);
        temp_bindingCount = BindRsp->bindingCount;
        for ( uint8 x = 0; x < bindingListCount; x++ ){
          uint16 sAd = BUILD_UINT16( BindRsp->list[x].srcAddr[0],BindRsp->list[x].srcAddr[1] );
          LREP("%d c=%X s=%X d=%X\r\n", x, BindRsp->list[x].clusterID, sAd, BindRsp->list[x].dstAddr.addr.shortAddr);

        }
        
        if (temp_bindingCount != 0 && temp_bindingCount - temp_bindingStartIndex > 3){
          LREPMaster("BIND_BIND_BIND\r\n");
          temp_bindingStartIndex = temp_bindingStartIndex +3;
          zclApp_RequestBind(1);
//          osal_start_timerEx(zclApp_TaskID, APP_REQ_BIND_EVT, 1000);
      ...
      LREP("InMsg->clusterID=0x%X InMsg->asdu=%X\r\n", InMsg->clusterID, InMsg->asdu);      
    {
      ...
      if (num != 0) {
        ...
        {
          if (count == 0){
            nwkDevAddr = _NIB.nwkDevAddress;
            
          } else {
            nwkDevAddr = temp_Sender_shortAddr[count];
          }
          if (LqRsp->list[i].nwkAddr == nwkDevAddr) {
            EndDevice_Lqi = LqRsp->list[i].lqi;
            temp_lqi[count] = EndDevice_Lqi;
            temp_LqiStartIndex[count] = LqRsp->startIndex;
            find = 1;
          } else {
...
void zclApp_RequestLqi(uint8 count){
  if ( bdbAttributes.bdbNodeIsOnANetwork ){
        ...
        startIndex = temp_LqiStartIndex[count];
        ZDP_MgmtLqiReq(&destAddr,startIndex, 0);
        temp_lqi[count] = 255;
        LREP("REQ_sI=%d\r\n", temp_LqiStartIndex[count]);
  }
}
#endif
...
//        destAddr.addr.shortAddr = 0x4436; 
        destAddr.addr.shortAddr = 0x4436;      
        startIndex = temp_bindingStartIndex;
    ...
    if (events & SYS_EVENT_MSG) {
        while ((MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive(zclApp_TaskID))) {          
            ...
            case ZDO_CB_MSG: // ZDO incoming message callback
                zclApp_ProcessZDOMsgs(temp_countReqLqi, (zdoIncomingMsg_t *)MSGpkt);
...
//        zclApp_GenTime_TimeUTC = osal_getClock();                
#ifdef LQI_REQ        
        zclApp_RequestLqi(temp_countReqLqi);
#endif 
#ifdef BIND_REQ
        temp_bindingCount = 0;
        temp_bindingStartIndex = 0;
...
static void zclApp_bh1750ReadLumosity(void) {
    ...
    if (temp_bh1750IlluminanceSensor_MeasuredValue[0] > zclApp_bh1750IlluminanceSensor_MeasuredValue){
      illum = (temp_bh1750IlluminanceSensor_MeasuredValue[0] - zclApp_bh1750IlluminanceSensor_MeasuredValue);
    } else {
      illum = (zclApp_bh1750IlluminanceSensor_MeasuredValue - temp_bh1750IlluminanceSensor_MeasuredValue[0]);
    }
...
//        LREP("Temperature=%d\r\n", zclApp_Temperature_Sensor_MeasuredValue);
        ...
        if (temp_Temperature_Sensor_MeasuredValue[0] > zclApp_Temperature_Sensor_MeasuredValue){
          temp = (temp_Temperature_Sensor_MeasuredValue[0] - zclApp_Temperature_Sensor_MeasuredValue);
        } else {
          temp = (zclApp_Temperature_Sensor_MeasuredValue - temp_Temperature_Sensor_MeasuredValue[0]);
        }
...
//        LREP("Pressure=%d\r\n", zclApp_PressureSensor_MeasuredValue);
        ...
        if (temp_PressureSensor_MeasuredValue[0] > zclApp_PressureSensor_MeasuredValue){
          press = (temp_PressureSensor_MeasuredValue[0] - zclApp_PressureSensor_MeasuredValue);
        } else {
          press = (zclApp_PressureSensor_MeasuredValue - temp_PressureSensor_MeasuredValue[0]);
        }
...
//        LREP("Humidity=%d\r\n", zclApp_HumiditySensor_MeasuredValue);
        ...
        if (temp_HumiditySensor_MeasuredValue[0] > zclApp_HumiditySensor_MeasuredValue){
          humid = (temp_HumiditySensor_MeasuredValue[0] - zclApp_HumiditySensor_MeasuredValue);
        } else {
          humid = (zclApp_HumiditySensor_MeasuredValue - temp_HumiditySensor_MeasuredValue[0]);
        }
...
static void EpdStatusLqiBatteryNwk(uint8 temp_s){
  ...
  if (zclApp_Config.HvacUiDisplayMode & 0x02){ // portrait
    ...
    EpdSetFrameMemoryXY(PaintGetImage(), 8, 120, PaintGetWidth(), PaintGetHeight());
    EpdSetFrameMemoryXY(PaintGetImage(), 8, 240, PaintGetWidth(), PaintGetHeight());
    EpdSetFrameMemoryXY(PaintGetImage(), 8, 360, PaintGetWidth(), PaintGetHeight());
  ...
  if (temp_s == 0) {
      temp_lqi[temp_s] = temp_lqi[0];
  }
  
  lqi_string[0] = temp_lqi[temp_s] / 100 % 10 + '0';
  lqi_string[1] = temp_lqi[temp_s] / 10 % 10 + '0';
  lqi_string[2] = temp_lqi[temp_s] % 10 + '0';
#if defined(EPD3IN7)
  if (zclApp_Config.HvacUiDisplayMode & 0x02){ // portrait
    if (temp_lqi[temp_s] != 255) { 
      PaintSetWidth(24);
      PaintSetHeight(16);
      PaintSetRotate(ROTATE_270);
      PaintSetInvert(!(zclApp_Config.HvacUiDisplayMode & 0x01));
      PaintClear(UNCOLORED);
      if(temp_lqi[temp_s] > 40){
        PaintDrawImage(IMAGE_LQI_100, 0, 0, 16, 24, COLORED);
      } else if (temp_lqi[temp_s] <= 40 && temp_lqi[temp_s] > 30) {
        PaintDrawImage(IMAGE_LQI_80, 0, 0, 16, 24, COLORED);
      } else if (temp_lqi[temp_s] <= 30 && temp_lqi[temp_s] > 20) {
        PaintDrawImage(IMAGE_LQI_60, 0, 0, 16, 24, COLORED);
      } else if (temp_lqi[temp_s] <= 20 && temp_lqi[temp_s] > 10) {
        PaintDrawImage(IMAGE_LQI_40, 0, 0, 16, 24, COLORED);
      } else if (temp_lqi[temp_s] <= 10 && temp_lqi[temp_s] > 0) {
        PaintDrawImage(IMAGE_LQI_20, 0, 0, 16, 24, COLORED);
      } else if (temp_lqi[temp_s] == 0) {
  ...
  // nwkDevAddress, nwkPanId, nwkLogicalChannel 
    char nwk_string[] = {'0', 'x', '0', '0', '0', '0', '\0'};
    if (temp_s == 0) {
      temp_Sender_shortAddr[temp_s] = _NIB.nwkDevAddress;
    }
    
    nwk_string[2] = temp_Sender_shortAddr[temp_s] / 4096 %16 + '0';
    if ((temp_Sender_shortAddr[temp_s]/4096 %16) > 9){
      nwk_string[2] = nwk_string[2]+7;
    }  
    nwk_string[3] = temp_Sender_shortAddr[temp_s] / 256 %16 + '0';
    if ((temp_Sender_shortAddr[temp_s]/256 %16) > 9){
      nwk_string[3] = nwk_string[3]+7;
    }
    nwk_string[4] = temp_Sender_shortAddr[temp_s] / 16 %16 + '0';
    if ((temp_Sender_shortAddr[temp_s]/16 %16) > 9){
      nwk_string[4] = nwk_string[4]+7;
    }
    nwk_string[5] = temp_Sender_shortAddr[temp_s] %16 + '0';
    if ((temp_Sender_shortAddr[temp_s] %16) > 9){
      nwk_string[5] = nwk_string[5]+7;
    }

#if defined(EPD3IN7)
  if (zclApp_Config.HvacUiDisplayMode & 0x02){ // portrait
    if (temp_Sender_shortAddr[temp_s] != 0xFFFE){  
      PaintSetWidth(72);
      PaintSetHeight(16);
      PaintSetRotate(ROTATE_0);
      PaintClear(UNCOLORED);
  ...
  char perc_string[] = {' ', ' ', ' ', ' ', '\0'};
  if (temp_s == 0) {
    temp_Battery_PercentageRemainig[temp_s] = zclBattery_PercentageRemainig;
  }
  
  if (temp_Battery_PercentageRemainig[temp_s] != 0xFF) {
    perc_string[0] = temp_Battery_PercentageRemainig[temp_s]/2 / 100 % 10 + '0';
    perc_string[1] = temp_Battery_PercentageRemainig[temp_s]/2 / 10 % 10 + '0';
    perc_string[2] = temp_Battery_PercentageRemainig[temp_s]/2 % 10 + '0';
    perc_string[3] = '%';
  }
#if defined(EPD3IN7)
  if (zclApp_Config.HvacUiDisplayMode & 0x02){ // portrait
    ...
    if (temp_Battery_PercentageRemainig[temp_s] != 0xFF) {
      ...
      PaintSetInvert(!(zclApp_Config.HvacUiDisplayMode & 0x01));
      PaintClear(UNCOLORED);
      if(temp_Battery_PercentageRemainig[temp_s]/2 > 75){
        PaintDrawImage(IMAGE_BATTERY_100, 0, 0, 16, 24, COLORED);
      } else if (temp_Battery_PercentageRemainig[temp_s]/2 <= 75 && temp_Battery_PercentageRemainig[temp_s]/2 > 50) {
        PaintDrawImage(IMAGE_BATTERY_75, 0, 0, 16, 24, COLORED);
      } else if (temp_Battery_PercentageRemainig[temp_s]/2 <= 50 && temp_Battery_PercentageRemainig[temp_s]/2 > 25) {
        PaintDrawImage(IMAGE_BATTERY_50, 0, 0, 16, 24, COLORED);
      } else if (temp_Battery_PercentageRemainig[temp_s]/2 <= 25 && temp_Battery_PercentageRemainig[temp_s]/2 > 6) {
        PaintDrawImage(IMAGE_BATTERY_25, 0, 0, 16, 24, COLORED);
      } else if (temp_Battery_PercentageRemainig[temp_s]/2 <= 6 && temp_Battery_PercentageRemainig[temp_s]/2 > 0) {
...
//  if (zclApp_Occupied_OnOff == 0) {
  ...
  if (temp_oc == 0){
   temp_Occupied[temp_oc] = zclApp_Occupied;
  }
...
{   
  ...
  for(uint8 i = 0; i <= 2; i++ ){  
    EpdStatusLqiBatteryNwk(i); // status network, lqi, perc battety, nwk adrr
...
//  HalLedSet(HAL_LED_1, HAL_LED_MODE_BLINK);
  uint8 i = 0;
  if (temp_Sender_shortAddr[1] == 0xFFFE) {
    i = 1;
    temp_Sender_shortAddr[i] = pInMsg->srcAddr.addr.shortAddr;
  }
  if (temp_Sender_shortAddr[1] != pInMsg->srcAddr.addr.shortAddr && temp_Sender_shortAddr[2] == 0xFFFE) {
    i = 2;
    temp_Sender_shortAddr[i] = pInMsg->srcAddr.addr.shortAddr;
  }
  
  if (temp_Sender_shortAddr[1] == pInMsg->srcAddr.addr.shortAddr) {
    i = 1;
  }
  if (temp_Sender_shortAddr[2] == pInMsg->srcAddr.addr.shortAddr) {
    i = 2;
  }
